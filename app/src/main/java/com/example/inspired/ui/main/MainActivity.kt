package com.example.inspired.ui.main
...
import com.example.inspired.databinding.ActivityMainBinding
import com.example.inspired.ui.base.BaseActivity
import com.example.inspired.ui.blog.BlogFragment
import com.example.inspired.ui.explore.ExploreFragment
import com.example.inspired.ui.home.HomeFragment
import com.example.inspired.ui.profile.ProfileFragment
            ...
            when (it.itemId) {
                R.id.tabFeed -> {
                    ...
                    if (fragNavController?.currentFrag is HomeFragment) {
                        Handler().postDelayed({ (fragNavController?.currentFrag as HomeFragment).onRefresh() }, 500)
                    }
                    fragNavController?.switchTab(TAB_1, FragNavTransactionOptions.newBuilder().build())
                }
                R.id.tabSearch -> {
                    fragNavController?.switchTab(TAB_2, FragNavTransactionOptions.newBuilder().build())
                }
                R.id.tabVideo -> {
                    fragNavController?.switchTab(TAB_3, FragNavTransactionOptions.newBuilder().build())
                }
                R.id.tabProfile -> {
                    fragNavController?.switchTab(TAB_4, FragNavTransactionOptions.newBuilder().build())
                }
            }
            true
        }
